...
bool checkSum(N *root,int key,int sum){
	...
	ans = ans || checkSum(root->left,key,sum);
	ans = ans || checkSum(root->right,key,sum);
	return ans;
}
...
int main(){
    ...
    cin>>key;
    if(checkSum(root,key,sum))	cout<<"Path EXISTS..."<<endl;
