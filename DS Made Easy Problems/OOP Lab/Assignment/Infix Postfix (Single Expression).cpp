...
{
    int i=0, j, m, n, x, ans, in[50];
    char a, b, post[50];

    cout<<"\n\nEnter values of variables for the above Postfix Expression:\n";
    for(int j=0;j<len;j++)      cin>>in[j];
    j=0;
    while(postfix[i]!='\0'){
        if(postfix[i]!='+' && postfix[i]!='-' && postfix[i]!='/' && postfix[i]!='*' && postfix[i]!='^')     postfix[i] = 48 + in[j++];
        i++;
    }
