...
void trim(N *root,int a,int b){
    if(!root)       return;
    if(root->data>a)        trim(root->left,a,b);
    if(root->data>=a && root->data<=b)  cout<<root->data<<"  ";
    if(root->data<b)        trim(root->right,a,b);
}
...
int main(){
    ...
    cout<<endl;
    int a,b;
    cout<<"Enter the range within which to trim the tree: ";
    cin>>a>>b;
    trim(root,a,b);
    return 0;
}
